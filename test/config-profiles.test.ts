...
const profiles = [
  { file: "config-development-host.json", env: "dev", mqttHost: "localhost", questdbUrl: "http://localhost:9000" },
  { file: "config-development-podman.json", env: "dev", mqttHost: "mosquitto", questdbUrl: "http://questdb:9000" },
  { file: "config-production.json", env: "prod", mqttHost: "mosquitto", questdbUrl: "http://questdb:9000" },
] as const;
...
test("configuration profiles are schema-valid and topology-specific", () => {
  for (const profile of profiles) {
    ...
    assert.equal(config.questdb.url, profile.questdbUrl);
